/**
 * LoginPage.jsx — Lovable-exact login form using Tailwind.
 */
...
import { login as content } from '../data/data.js';
...
export default function LoginPage() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!email || !password) { showToast(content.toasts.missingInfo); return; }
        setLoading(true);
        try {
            const res = await authApi.login({ email, password });
            login(res.data.access_token, res.data.user);
            showToast(content.toasts.welcome);
            setTimeout(() => navigate('/dashboard'), 500);
        } catch {
            showToast(content.toasts.invalidCredentials);
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="glass-card-enhanced rounded-2xl p-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <button type="submit" disabled={loading}
                        ...
                        {loading ? 'Signing in…' : `${content.form.submitButton} →`}
                    </button>
                </form>

                <p className="text-center text-sm text-muted-foreground mt-5">
                    {content.form.noAccount}{' '}
                    <Link to="/register" className="text-primary hover:text-primary/80 font-semibold no-underline">{content.form.signUpLink}</Link>
                </p>
            </div>
        </AuthLayout>
    );
